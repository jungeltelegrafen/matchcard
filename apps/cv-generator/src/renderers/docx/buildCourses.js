...
export function buildCourses(items, lang = 'en') {
  if (!items?.length) return []
  const lb = getL(lang)

  return [
    sectionHeading(lb.courses),
    ...items.map((item, idx) => {
      const nameRuns = [
        new TextRun({ text: item.name || '', size: 20, bold: true, font: 'Calibri', color: hex(theme.colors.text) }),
        item.institution ? new TextRun({ text: `  —  ${item.institution}`, size: 18, color: hex(theme.colors.muted), font: 'Calibri' }) : null,
      ].filter(Boolean)
