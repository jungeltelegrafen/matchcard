...
// Source text beyond this limit is truncated before parsing. Generous because
// Sonnet's context easily fits large CVs — the real bottleneck is OUTPUT tokens
// per call (handled by the two-phase parse pipeline), not input size. The client
// warns the user when their input exceeds it.
export const PARSE_CHAR_LIMIT = 100000
...
export const CV_SECTIONS = {
  ...
  personal: {
    ...
    fields: {
      ...
      summary: { type: str, desc: 'Weighted professional summary (3–5 sentences) synthesizing the WHOLE career: seniority, core domains/roles, strongest technologies, and standout achievements — not just the opening lines of the source' },
    },
  },
  ...
  experience: {
    ...
    itemFields: {
      ...
      endDate: str,
      location: str,
      description: { type: str, desc: 'Description of THIS single project/engagement (context). One entry per distinct project — never merge multiple projects into one entry.' },
  ...
  skills: {
    ...
    itemFields: {
      ...
      items: { type: strArr, desc: 'Skill tags — always an array of strings, never a comma-separated string. Curate: deduplicate, prefer recurring/relevant skills, keep it focused (roughly ≤12 per group) rather than an exhaustive dump' },
    },
  },
  ...
  competences: {
    ...
    flags: {
      // Opt-out: competences are included by default and only actually render
      // when at least one row has a requirement. Toggling this off explicitly
      // excludes the section even when filled.
      enabled: { type: bool, default: true },
      simpleFormat: { type: bool, default: false },
      projectLabel: str,
    },
  ...
  portfolio: {
    ...
    itemFields: {
      label: { type: str, desc: 'Title/name for this link, e.g. "GitHub", "Personal site", "Design portfolio"' },
      category: { type: str, desc: 'Kind of portfolio — one of: code, design, project, writing, other' },
      url: str,
      description: str,
    },
  },
  ...
  videos: {
    ...
    itemFields: {
      ...
      anchor: { type: str, desc: 'Unit this video renders in: a section key (summary|skills|competences|education|courses|portfolio) or an experience/position item _id. Empty = main video block.' },
      placement: { type: str, default: 'general', desc: 'DEPRECATED cosmetic tag (no longer shown)' },
      experienceId: { type: str, desc: 'DEPRECATED — legacy experience anchor, read as fallback for `anchor`' },
...
// ── Two-phase parse schemas ──────────────────────────────────────────────────
// Phase 1 (outline) returns holistic sections in full plus experience/positions
// as HEADERS ONLY, so output stays small. Phase 2 expands one section's items to
// full detail in parallel batches.

const EXPERIENCE_HEADER = ['company', 'role', 'startDate', 'endDate', 'location']
const POSITION_HEADER   = ['company', 'title', 'startDate', 'endDate']

function pickItemFields(itemFields, keys) {
  return Object.fromEntries(keys.filter(k => itemFields[k]).map(k => [k, itemFields[k]]))
}

// The compact outline tool schema for phase 1.
export function buildOutlineSchema() {
  ...
  return {
    ...
    additionalProperties: false,
    properties: {
      personal:       objectSchema(S.personal.fields),
      education:      { type: 'array', items: objectSchema(S.education.itemFields) },
      skills:         { type: 'array', items: objectSchema(S.skills.itemFields) },
      languages:      { type: 'array', items: objectSchema(S.languages.itemFields) },
      certifications: { type: 'array', items: objectSchema(S.certifications.itemFields) },
      courses:        { type: 'array', items: objectSchema(S.courses.itemFields) },
      experience:     { type: 'array', items: objectSchema(pickItemFields(S.experience.itemFields, EXPERIENCE_HEADER)) },
      positions:      { type: 'array', items: objectSchema(pickItemFields(S.positions.itemFields, POSITION_HEADER)) },
    },
  }
}

// A single-section array tool schema (full item fields) for phase-2 expansion.
export function buildSectionArraySchema(sectionKey) {
  const def = CV_SECTIONS[sectionKey]
  return {
    type: 'object',
    additionalProperties: false,
    properties: { [sectionKey]: { type: 'array', items: objectSchema(def.itemFields) } },
  }
}
